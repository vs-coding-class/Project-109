...
mathScores = df2["math score"].to_list()

mean = statistics.mean(mathScores)
median = statistics.median(mathScores)
mode = statistics.mode(mathScores)
...
def standardDeviation():
    ...
    standardDeviation = math.sqrt(numeratorPart/len(mathScores))
...
figure = ff.create_distplot([mathScores], ["Result"], show_hist = False)
...
sigmaResults = [result for result in mathScores if result > sigmaStart and result < sigmaEnd]
sigma2Results = [result2 for result2 in mathScores if result2 > sigma2Start and result2 < sigma2End]
sigma3Results = [result3 for result3 in mathScores if result3 > sigma3Start and result3 < sigma3End]

sigmaPercentage = len(sigmaResults)/len(mathScores)*100
sigma2Percentage = len(sigma2Results)/len(mathScores)*100
sigma3Percentage = len(sigma3Results)/len(mathScores)*100
